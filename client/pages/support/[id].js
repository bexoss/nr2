...
export default function SupportTicketDetail() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`http://localhost:4000/api/support/tickets/${id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const close = useMutation({
    mutationFn: async () => {
      const res = await fetch(`http://localhost:4000/api/support/tickets/${id}/close`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error('Failed');
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['support','ticket', id] })
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6">
      <div className="mb-3"><a href="/support" className="text-gray-700 underline">← 목록으로</a></div>
      {q.isLoading ? 'Loading…' : q.isError ? 'Error' : (
        <div>
          ...
          <form className="mt-4 space-y-2" onSubmit={(e) => { e.preventDefault(); const text = e.currentTarget.message.value; if (!text) return; post.mutate({ text }); e.currentTarget.reset(); }}>
            <textarea name="message" className="w-full border rounded px-3 py-2 min-h-[100px]" placeholder="답변 또는 추가 정보를 입력하세요" />
            <div className="flex gap-2">
              <button className="px-4 py-2 rounded bg-gray-900 text-white" type="submit">보내기</button>
              {q.data.status!=='closed' && <button className="px-4 py-2 rounded bg-gray-100 text-gray-900" type="button" onClick={() => close.mutate()}>티켓 종료</button>}
            </div>
          </form>
        </div>
      )}

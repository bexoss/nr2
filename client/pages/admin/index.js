...
export default function AdminPage() {
  ...
  if (!token) return <main className="max-w-6xl mx-auto px-4 py-6">Login required</main>
  if (!me?.isAdmin) return <main className="max-w-6xl mx-auto px-4 py-6">Admin only</main>
    ...
    <main className="max-w-6xl mx-auto px-4 py-6 flex gap-6">
      <aside className="w-60 border-r border-gray-100 pr-4">
        ...
        <nav className="flex flex-col gap-2">
          <button className={`text-left ${tab==='users'?'font-semibold':''}`} onClick={() => setTab('users')}>Users</button>
          <button className={`text-left ${tab==='products'?'font-semibold':''}`} onClick={() => setTab('products')}>Products</button>
          <button className={`text-left ${tab==='orders'?'font-semibold':''}`} onClick={() => setTab('orders')}>Orders</button>
        </nav>
      </aside>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <a href="/admin/products/new" className="px-3 py-2 rounded bg-gray-900 text-white">New Product</a>
      </div>
      {q.isLoading ? 'Loading...' : q.isError ? 'Error' : (
        <div className="space-y-2">
          {q.data.map((p) => (
            <div key={p.sku} className="border border-gray-100 rounded p-2 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                <a className="px-2 py-1 rounded bg-gray-100" href={`/admin/products/${p.sku}`}>Edit</a>
                <button className="px-2 py-1 rounded bg-rose-600 text-white" onClick={() => deleteMutation.mutate(p.sku)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {q.data.map((o) => (
            <div key={o._id} className="border border-gray-100 rounded p-2 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button className="px-2 py-1 rounded bg-gray-100" onClick={() => patch.mutate({ id: o._id, status: 'shipped' })}>Ship</button>
                <button className="px-2 py-1 rounded bg-gray-100" onClick={() => patch.mutate({ id: o._id, status: 'delivered' })}>Deliver</button>
              </div>
            </div>
          ))}
        </div>
